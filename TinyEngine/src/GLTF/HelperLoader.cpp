...
bool HelperLoader::initImages(GLTF& glTF, const std::string& path)
{
	for (size_t i = 0; i < model.images.size(); i++)
	{
		...
		if (!FileIO::openImageData(image.imageDataResources, path + image.uri))
		{
			return false;
		}
	...
	for (size_t i = 0; i < model.materials.size(); i++)
	{
		...
		for (const auto& parameterValue : model.materials[i].values)
		{
			if (parameterValue.first == "baseColorTexture")
			{
				...
				material.macros["BASECOLOR_TEXCOORD"] = HelperAccess::getTexCoord(material.pbrMetallicRoughness.baseColorTexture);
			...
			if (parameterValue.first == "metallicRoughnessTexture")
			{
				...
				material.macros["METALLICROUGHNESS_TEXCOORD"] = HelperAccess::getTexCoord(material.pbrMetallicRoughness.metallicRoughnessTexture);
		...
		for (const auto& parameterValue : model.materials[i].additionalValues)
		{
			if (parameterValue.first == "emissiveTexture")
			{
				...
				material.macros["EMISSIVE_TEXCOORD"] = HelperAccess::getTexCoord(material.emissiveTexture);
			...
			if (parameterValue.first == "occlusionTexture")
			{
				...
				material.macros["OCCLUSION_TEXCOORD"] = HelperAccess::getTexCoord(material.occlusionTexture);
			...
			if (parameterValue.first == "normalTexture")
			{
				...
				material.macros["NORMAL_TEXCOORD"] = HelperAccess::getTexCoord(material.normalTexture);
...
bool HelperLoader::open(GLTF& glTF, const std::string& filename, const std::string& environment, bool useRaytrace)
{
	...
	if(!FileIO::openImageData(diffuseMap.imageDataResources, diffuseFilename))
	{
		return false;
	}
	...
	if(!FileIO::openImageData(specularMap.imageDataResources, specularFilename))
	{
		return false;
	}
	...
	if(!FileIO::openImageData(lutMap.imageDataResources, lutFilename))
	{
		return false;
	}
